...
Status: internal leading-cell construction and obstruction.  This is not a
Navier--Stokes singularity proof.  The original frozen-symbol statement that
the nonlinear-phase high sum was *exactly* pressure has been corrected below;
the full formulas and viscous-capture theorem are in
...
* no simultaneous finite-frequency two-colour cell cancels all `O(Q)` mixed
  bands while retaining both desired low beats.
...
## 4. Exact envelope cell equation and a core-buffer geometry
...
the non-pressure part of their common high sum vanishes exactly iff
...
At `H=-1` the dark direction is the common direction `n`.  Hence the carrier
can have a compact Gevrey lateral cutoff while remaining constant along `n`
through the active core.  A longitudinal cutoff necessarily violates the
dark equation; place it in remote endcaps of a long buffer tube.  If the
endcap length is `L_b`, its high-sum defect relative to the low beat is
`O((Q L_b)^(-1))`, before viscous gain.  This is a separated wake rather than
a core error.

Simultaneous colours have the unavoidable mixed-band obstruction above.
The clean protocol is sequential:
...
The exact algebra supports a minimal two-colour strain dictionary and an
exact nonlinear-phase affine writer.  It also proves simultaneous exact
finite-colour cancellation impossible at the first nontrivial `O(Q)` order.
The viable PDE lemma must therefore be sequential and must quantify:
