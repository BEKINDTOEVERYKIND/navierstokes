...
log-periodic *stationary outer wake*.  The Liouville question posed in the
first version has since been answered constructively: disjoint dilates of
a compactly supported Gavrilov steady Euler bubble give an explicit smooth
periodic cylinder solution.  The construction is recorded in
[`2026-07-29-gavrilov-dss-wake-construction.md`](2026-07-29-gavrilov-dss-wake-construction.md).
Neither note constructs a Navier--Stokes singularity.
...
5. Irrotational fields, all generalized Beltrami fields, purely radial
   ...
   fields, and a perturbative branch from zero are all excluded.  These
   exclusions do not imply a Liouville theorem: the Gavrilov lattice is a
   nonperturbative periodic shape with disjoint compact components and
   identically empty radial gaps.
6. Reflection-paired, generically placed bubbles make the wake
   nonaxisymmetric and cancel helicity exactly in each logarithmic cell.
   The remaining prize-level obstruction is the viscous, time-dependent
   annular transition, not existence of the stationary wake.
...
Equation (7.6) still leaves open a nonseparable *everywhere tangential*
periodic path through the infinite-dimensional set of steady Euler flows
on \(\mathbb S^2\).  This is no longer needed for wake existence.  The
Gavrilov lattice instead has both radial and tangential velocity inside
compact annular bubbles and vanishes on open cylinder gaps.  It is
nonperturbative and is not generated by a simple rotation or amplitude
modulation.
...
window.  The Gavrilov lattice shows that this says nothing about the full
periodic cylinder problem: a periodic orbit with open zero-field gaps need
not bifurcate from a homogeneous equilibrium.  Floquet analysis is
therefore optional structure, not an existence gate.
...
asymptotically discretely self-similar modulation.  The Gavrilov lattice
now supplies the leading Euler wake.  Formula (9.3) is useful only after a
Gevrey-tame corrector theory for viscosity and the active annular
transition has been proved.

## 10. The wake theorem is solved; the transition theorem is not

Let \((W,P)\) be a nonzero smooth compactly supported steady Euler pair,
with joint support in a strict radial subannulus satisfying \(b/a<r\).
For every \(\gamma\), the locally finite sums
\[
 U(x)=\sum_{j\in\mathbb Z}r^{\gamma j}W(r^jx),
 \qquad
 \Pi(x)=\sum_{j\in\mathbb Z}r^{2\gamma j}P(r^jx)
\tag{10.1}
\]
have disjoint supports, solve stationary Euler exactly on
\(\mathbb R^3\setminus\{0\}\), and obey
\[
 U(rx)=r^{-\gamma}U(x),\qquad
 \Pi(rx)=r^{-2\gamma}\Pi(x).
\tag{10.2}
\]
Thus (2.7) gives the required nonconstant periodic cylinder solution.
Reflection-paired generic placements impose orientation-reversing
zero-helicity symmetry and remove global axisymmetry.  Section 5's
zero-flux conditions hold automatically because every log cell contains a
spherical gap on which both \(U\) and \(\Pi\) vanish.

The decisive next statement is instead:

> **Active-transition theorem target.**  Starting from one smooth
> preterminal Navier--Stokes cell, construct a time-dependent annular
> transition that creates the next smaller Gavrilov bubble, drains the
> correct amount of parent energy, transports all centre/carrier modes,
> and incorporates the viscous endpoint jets with Gevrey bounds strong
> enough to leave an \(e^{-c j^2}\) residual and seam error.

The stationary bubbles cannot perform this transfer: their energy flux is
exactly zero and their bare viscous residual is
\(-\nu\Delta W_j\).  A GPU search for cylinder-wake existence is therefore
obsolete.  Computation becomes relevant only after a finite-dimensional
active-transition reduction or a discretized inverse with an analytically
controlled continuum limit is available.
